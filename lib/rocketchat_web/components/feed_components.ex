    ...
    ~H"""
      <div class="flex flex-col gap-5 pt-5 bg-white sticky top-0 z-10">
        ...
        <div class="flex justify-center gap-10 px-20">
          <button phx-click="switch_tabs" phx-value-tab="fyp" class={if @tab === :fyp, do: active_tab_button_class}>For You</button>
          <button phx-click="switch_tabs" phx-value-tab="following" class={if @tab === :following, do: active_tab_button_class}>Following</button>
        </div>
      </div>
    """
  end
    ...
    ~H"""
      <div class="flex px-3 gap-3 items-center pt-5 pb-2">
        ...
        <div class="rounded-2xl shadow-2xl bg-white px-4 py-5 flex flex-col w-full">
          <h3>John</h3>
          <input placeholder="New post to followers..." class="placeholder:text-slate-400 focus:outline-none" />
        </div>
      </div>
    """
  end
    ...
    ~H"""
      ...
      <div class="flex px-3 py-5 gap-3 bg-[#f7f8fa] border-y-[#efefef] border-y">
        ...
        <div class="flex flex-col relative items-end">
          ...
          <button class="relative bottom-3 -right-1 rounded-full bg-sky-500 h-5 w-5 text-[0.7rem] text-white font-bold leading-none align-middle flex items-center justify-center border border-white">+</button>
        </div>
