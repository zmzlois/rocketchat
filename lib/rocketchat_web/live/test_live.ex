defmodule RocketchatWeb.TestLive do
  ...
  def render(assigns) do
    ...
    <.form 
      for={@form}
      phx-submit="prompt"
      class="flex flex-col gap-2 justify-center item-center w-screen h-screen bg-purple-100"
    >
      ...
      <div class="flex justify-center">
        <button class="border-2 border-neutral-300 px-10 py-2 text-neutral-600 hover:bg-teal-300 transition-all hover:text-white rounded-lg">Prompt</button>
      </div>

      <div 
        :if={String.length(@transcribed) > 0} 
      ...
      <div 
        :if={String.length(@summerized_transcription) > 0} 
      ...
      <div 
        :if={String.length(@summerized_prompt) > 0} 
        class="flex justify-center flex-col"
      >
        <h3>Summary of transcription:</h3>
        <%= @summerized_prompt %>
